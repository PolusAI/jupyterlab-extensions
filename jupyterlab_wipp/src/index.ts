...
function registerByPath(path: string): void {
  ...
  showDialog({
    ...
    console.debug(
      `Form accepted. Name: ${info.name}. Description: ${info.description}`
    );

    // Launch WippRegister dialog
    showDialog({
      title: 'Waiting for WIPP...',
      body: new WippRegister(
        path,
        info.name,
        info.description,
        info.openInWipp
      ...
      .then(response => {
        ...
        if (response.code === 200) {
          ...
          app.commands.addCommand(registerContextMenuCommandID, {
            ...
            execute: () =>
              registerByPath(
                factory.tracker.currentWidget.selectedItems().next().path
              )
          });
          ...
          app.commands.addCommand(registerFileMenuCommandID, {
            ...
            isVisible: () =>
              ...
              notebookTracker.currentWidget === app.shell.currentWidget, // Check if notebook is open to enable menu command
            execute: () =>
              registerByPath(notebookTracker.currentWidget.context.path)
          });
