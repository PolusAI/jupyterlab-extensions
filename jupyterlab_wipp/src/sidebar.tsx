...
export class WippSidebar extends Widget {
  ...
  constructor(
    ...
    this._search = new SearchWidget(
      () => this.get_search_placeholder(),
      arg0 => this._searchCollections(arg0)
    );
    layout.addWidget(this._search);

    // Add buttons to choose type of WIPP Collection
    this._switcher = new SwitcherWidget(arg0 =>
      this.swithchCollectionType(arg0)
    );
  ...
  private swithchCollectionType(choice: number) {
    ...
    const consoleTracker = this.consoleTracker;
  ...
  private async _getUiUrls() {
    ...
    requestAPI<any>('ui_urls', {}).then(response => {
  ...
  private async _getAllCollections() {
    ...
    requestAPI<IGenericCollection[]>(this._get_collection_type_url, {}).then(
      objectArray => {
  ...
  private async _searchCollections(name: string) {
    ...
    requestAPI<IGenericCollection[]>(
      this._search_collection_type_url,
      fullRequest
    ).then(objectArray => {
